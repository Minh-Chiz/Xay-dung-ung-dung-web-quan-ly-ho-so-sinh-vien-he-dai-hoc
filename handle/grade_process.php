...
switch ($action) {
    case 'create':
        checkRole(['teacher','admin'], '../views/grade.php', "Bạn không có quyền thêm điểm số."); 
        handleCreateGrade();
        break;
    case 'edit':
        checkRole(['teacher', 'admin'], '../views/grade.php', "Bạn không có quyền chỉnh sửa điểm số."); 
        handleEditGrade();
        break;
    case 'delete':
        checkRole(['teacher','admin'], '../views/grade.php', "Bạn không có quyền xóa điểm số."); 
        handleDeleteGrade();
        break;
    default:
        break;
...
/**
 * Lấy tất cả danh sách điểm số (có hỗ trợ lọc)
 */
function handleGetAllGrades($student_id_filter = null, $search_code = '', $search_class = '') {
    // Truyền thêm 2 tham số tìm kiếm xuống hàm xử lý
    return getAllGrades($student_id_filter, $search_code, $search_class);
}
...
function handleEditGrade() {
    ...
    if ($id <= 0 || $student_id <= 0 || $subject_id <= 0 || $grade < 0 || $grade > 10) {
        header("Location: ../views/grade/edit_grade.php?id=" . $id . "&error=Dữ liệu không hợp lệ.");
        exit();
    }
